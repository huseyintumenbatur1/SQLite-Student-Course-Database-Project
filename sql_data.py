...
def basic_sample_data(cursor):
    ...
    # 4) Where clause
    print("--------- Where White String = New York ----------")
...
def sql_update_delete_insert_operation(conn, cursor):

    #Insert
    cursor.execute("INSERT INTO Students VALUES (6,'FRANK BLBLBL', 23, 'frank@gmail.com', 'Miami')")
    conn.commit()
...
def sql_aggregate(cursor):
    print("----------- AGGREGATE Count--------------")
    # count
    cursor.execute("SELECT COUNT(*) FROM Students")
    resualt = cursor.fetchone()
    print(resualt[0])
...
def main():
    ...
    try:
        ...
        basic_sample_data(cursor)
        sql_update_delete_insert_operation(conn, cursor)
        sql_aggregate(cursor)
        answers(cursor)
        conn.commit()
